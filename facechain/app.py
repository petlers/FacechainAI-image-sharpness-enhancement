...
def launch_detect(uuid, base_model_index, template_image, test_img_folder):
    
    device = torch.device('cpu')  # if you want to run on CPU, change 'cuda' -> cpu
    model_path =  'models/RRDB_ESRGAN_x4.pth' # models/RRDB_ESRGAN_x4.pth OR models/RRDB_PSNR_x4.pth
    # test_img_folder = 'LR/comic.jpg'
    ...
    idx = 0
    for path in glob(test_img_folder):
        idx += 1
...
def detect_picture():
    ...
    preset_template = glob(os.path.join('LR/*.jpg'))
    with gr.Blocks() as demo:
        uuid = gr.Text(label="modelscope_uuid", visible=False)
        # Initialize the GUI
        ...
        with gr.Row():
            with gr.Column():
                with gr.Box():
                    ...
                    template_image_list = [[i] for idx, i in enumerate(preset_template)]
                    print(template_image_list)
                    template_image = gr.Image(source='upload', type='filepath', label='图片(Template image)')
                    gr.Examples(template_image_list, inputs=[template_image], label='模板示例(Template examples)')
                    
                    test_img_folder = template_image
                
                base_model_list = [
                    {"name": "RRDB_ESRGAN_x4.pth", "path": "facechain/models/RRDB_ESRGAN_x4.pth"},
                    {"name": "RRDB_PSNR_x4.pth", "path": "facechain/models/RRDB_PSNR_x4.pth"}
                ]

                base_model_names = [base_model['name'] for base_model in base_model_list]
        ...
        display_button.click(
            fn=launch_detect,
            inputs=[uuid, base_model_index, template_image, test_img_folder],
            outputs=[infer_progress, output_images]
        )
        
        # Get the generated images from launch_detect function
        output_images.srcs = launch_detect
...
with gr.Blocks(css='style.css') as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem('\N{party popper}固定模板形象写真(Fixed Templates Portrait)'):
            inference_inpaint()
        with gr.TabItem('\N{party popper}人脸及图像清晰化修补(image and facial inpainting inpainting)'):
            detect_picture()
